...
export function left(position: vscode.Position, count = 0): vscode.Position {
  return position.with({
    character: Math.max(position.character - count, 0),
  });
}

export function right(document: vscode.TextDocument, position: vscode.Position, count = 0): vscode.Position {
...
export function rightNormal(document: vscode.TextDocument, position: vscode.Position, count = 0): vscode.Position {
  const lineLength = document.lineAt(position.line).text.length;
